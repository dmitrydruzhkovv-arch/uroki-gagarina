...
(function(){
  ...
  function fxRender(svg){
    ...
    (cfg.pts||[]).forEach(function(p){
      ...
      svg.appendChild(E('circle',p.o?{cx:X(p.x),cy:Y(p.y),r:p.r||4,fill:p.see?'none':'#fff',stroke:p.col||'#d97706','stroke-width':1.8}:{cx:X(p.x),cy:Y(p.y),r:p.r||4,fill:p.col||'#d97706'}));
