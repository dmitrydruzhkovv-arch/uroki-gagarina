...
let sloy = 0;
function fraza(kid, n, tekst, i){
  const el = document.createElement('div');
  el.className = 'salyut-fraza k-' + kid;
  el.innerHTML = `<span class="sf-e">${PICK(EMOJI)}</span><span class="sf-t">${tekst(n)}</span>`;
  /* первая — в центре, следующие накидываются сверху со сдвигом и своим наклоном */
  el.style.setProperty('--rot', R(i ? -14 : -5, i ? 14 : 5).toFixed(1) + 'deg');
  const uzko = innerWidth < 700;                                   // телефон: плашки не вылезают за край
  el.style.left = (50 + (i ? R(uzko ? -5 : -22, uzko ? 5 : 22) : 0)) + '%';
  el.style.top  = (38 + (i ? R(-22, 26) : 0)) + '%';
  el.style.zIndex = 9999 + (++sloy);
...
/* Объём «как десять нажатий подряд» в одно закрытие (D, 17.09.2026):
   ...
   нажатия, остальные — из случайных мест экрана. На первые семь залпов —
   по фразе: накидываются друг на друга, не повторяясь. */
window.SALYUT = (kid, x, y, vsego) => {
  ...
  const frazy = FRAZY.slice().sort(() => Math.random() - 0.5);     // без повторов в одном празднике
  if (tiho){ fraza(kid, vsego, frazy[0], 0); return; }
  ...
  for (let i = 0; i < zalpov; i++){
    ...
    setTimeout(() => { fn(cv, ex, ey); if (i < 7) fraza(kid, vsego, frazy[i], i); }, t);
    t += R(90, 260);
  }
};
})();
